...
MOD_CODE = os.getenv("MOD_CODE", "8647")
...
# --- Inventory JotForm ---
GEN_FORM_URL = "https://submit.jotform.com/submit/YOUR_GEN_FORM"
AW_FORM_URL = "https://submit.jotform.com/submit/YOUR_AW_FORM"
...
async def inventory(ctx, company: str):
    ...
    if company == "gen":
        responses["Company"] = "GENMOBILE"
        jotform_url = GEN_FORM_URL
    elif company == "aw":
        responses["Company"] = "ASSURANCE"
        jotform_url = AW_FORM_URL
    else:
        await ctx.author.send("❌ Invalid company. Use `gen` or `aw`.")
        return
    ...
    responses["First Name"] = first_name
    responses["Last Name"] = last_name
    responses["Agent Email"] = "chriscant.inventory@gmail.com"
    responses["Special Notes (optional)"] = "Generated by Lumina (c) Chris Cant 2025"

    def check(m):
        return m.author == user and isinstance(m.channel, discord.DMChannel)

    try:
        await user.send("Do you have RMAs? (YES/NO)")
        rma_msg = await bot.wait_for("message", check=check, timeout=300)
        has_rma = rma_msg.content.strip().lower() == "yes"

        if has_rma:
            await user.send("Please provide IMEIs for RMAs (one per line):")
            imei_msg = await bot.wait_for("message", check=check, timeout=600)
            responses["RMAs (if any, optional)"] = imei_msg.content
        else:
            responses["RMAs (if any, optional)"] = "N/A"

        signature_b64 = generate_signature(first_name, last_name)
        payload = {
            FIELD_MAPPING["inventory"]: "YES",
            FIELD_MAPPING["company"]: responses["Company"],
            FIELD_MAPPING["first_name"]: first_name,
            FIELD_MAPPING["last_name"]: last_name,
            FIELD_MAPPING["email"]: responses["Agent Email"],
            FIELD_MAPPING["imeis"]: responses["RMAs (if any, optional)"],
            FIELD_MAPPING["rmas"]: responses["RMAs (if any, optional)"],
            FIELD_MAPPING["notes"]: responses["Special Notes (optional)"],
            FIELD_MAPPING["signature"]: signature_b64,
            FIELD_MAPPING["date"]: datetime.now().strftime("%m%d%Y"),
            "formID": "231344559880059",
            "submitSource": "Lumina Bot",
        ...
        if r.status_code == 200:
            inventory_data[str(user.id)] = {
                ...
                "imeis": len(responses["RMAs (if any, optional)"].splitlines()),
